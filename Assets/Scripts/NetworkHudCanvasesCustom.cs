...
public class NetworkHudCanvasesCustom : MonoBehaviour   //can't inherit bc variables are privated; Only OnGUI part is active for InputSystem
{
    #region Types.
    /// <summary>
    ...
    void OnGUI()
    {
        ...
        {
            if (GUILayout.Button($"{GetNextStateText(_serverState)} Host", GUILayout.Width(buttonSize.x), GUILayout.Height(buttonSize.y)))
            {
                OnClick_Server();
                OnClick_Client();
            }    
